...
display_winner([_, _, _, _, WS, BS], W) :-
    nl, nl,
    (W == 'Tie' ->
...
display_period_score([_, _, _, R, NWS, NBS]) :-
...
display_choose_move(CP) :-
    ...
    nl,
    write('Choose a Card (1, 2, 3, or 4) - Starting Square - End Square (e.g., 1-A4-B4): '), nl,        
    (CP == 'w' ->
        write('White player, choose a move for the black player: ')
    ; CP == 'b' ->
        write('Black player, choose a move for the white player:  ')
    ). 
