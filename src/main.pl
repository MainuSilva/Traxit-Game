...
get_player_mode(Player, Mode):-
    repeat,
    display_player_modes(Player),
    read(Mode),
    catch(char_code(_, Mode), _, fail),
    valid_player_mode(Mode).
...
play_game([_, _, _, R, WS, BS]):-
    (R > 16 ->
        game_over([_, _, _, R, WS, BS], W),
        display_winner([_, _, _, R, WS, BS], W),
        retractall(state(_)),
        retractall(player(_,_))
    ;
        NR is R + 1,
        play_round([_, _, _, NR, WS, BS], NGS),
        assertz(state(NGS)),
...
get_move(M):-
    ...
    (parse_move(AN, M) -> !
    ;
     (write('Invalid algebraic notation. Try again\n'), get_move(M))
    ).
...
try_move([CP, _, _, _,_ ,_], M, NGS):-
   (move([CP, _, _, _,_ ,_], M, NGS)-> !
   ;
    (write('Invalid move!\n'), fail)
   ).
...
play_round([CP, _, _, _,_ ,_], NGS) :-
    get_current_player([CP, _, _, _,_ ,_], P1),
    display_board([P1, _, _, _,_ ,_]),
    display_choose_move(CP), % Last player chooses next player's move.
    repeat,
    catch((get_move([P1, _, _, _,_ ,_], Move)),_,(write('Invalid input. Try again\n'), fail)),
    try_move([P1, _, _, _,_ ,_], Move, NGS),
    skip_line, !.

                           
% Define a predicate to get the current player based on the round.
get_current_player([CP, _, _, R, _,_], NP) :-
    (R =:= 1 -> 
